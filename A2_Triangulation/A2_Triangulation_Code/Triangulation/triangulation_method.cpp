...
) const
{
    ...
    // ----------------------------------------------------------------------
    // Step 6 (Evaluation): Compute the average re-projection error, and store reprojected 2D points in memory
    // ----------------------------------------------------------------------
    {
        // 1) Preparing the projection matrix
        //    P1: first camera (K[I|0]), constructed above => constructProjectionMatrix(K, I_mat, zero)
        //    P2_final: second camera (K[R|t]), constructed above => constructProjectionMatrix(K, R, t)

        double total_error = 0.0;
        int N_pts = points_3d.size();

        // Create a container to store the reprojected points of the two images
        std::vector<Vector2D> reprojected_0(N_pts), reprojected_1(N_pts);

        for (int i = 0; i < N_pts; ++i) {
        
        // 2) Take the i-th triangulated 3D point (non-homogeneous)
            Vector3D X3D = points_3d[i];

        //  Expands to homogeneous coordinates (X, Y, Z, 1)
            Vector4D Xh = X3D.homogeneous();

        // 3) Projection to the first image
            Vector3D uv1 = P1 * Xh; // 得到 (u, v, w)
            double u1 = uv1[0] / uv1[2];
            double v1 = uv1[1] / uv1[2];

        // 4) Calculate the Euclidean distance to the original pixel points_0[i]
            double err1 = std::sqrt(
                (u1 - points_0[i][0]) * (u1 - points_0[i][0]) + (v1 - points_0[i][1]) * (v1 - points_0[i][1]));

        // 5) Projection to the second image
            Vector3D uv2 = P2_final * Xh; // (u, v, w)
            double u2 = uv2[0] / uv2[2];
            double v2 = uv2[1] / uv2[2];

        // Calculate the Euclidean distance to the original pixel points_1[i]
            double err2 = std::sqrt(
                (u2 - points_1[i][0]) * (u2 - points_1[i][0]) +
                (v2 - points_1[i][1]) * (v2 - points_1[i][1])
            );

        // 6) Accumulate the error (here we simply average the error between the two images)
            double avg_err_i = 0.5 * (err1 + err2);
            total_error += avg_err_i;

        // 7) Save Reprojected Points
            reprojected_0[i] = Vector2D(u1, v1);
            reprojected_1[i] = Vector2D(u2, v2);
        }

        double mean_error = total_error / N_pts;
        std::cout << "---------------------------------------------------\n";
        std::cout << "Average re-projection error = " << mean_error << " pixels\n";
        std::cout << "---------------------------------------------------\n";


        // -----------------------------------------
        // Step 7: Output reprojected_0 and reprojected_1 to .xyz files
        // -----------------------------------------
        {
            std::ofstream ofs0("reprojected_0.xyz");
            std::ofstream ofs1("reprojected_1.xyz");

            if (!ofs0 || !ofs1) {
                std::cerr << "Failed to open output file for reprojected points\n";
            }
            else {
                // Write out the 2D point as (x, y, 0)
                for (int i = 0; i < N_pts; ++i) {
                    ofs0 << reprojected_0[i][0] << " "
                        << reprojected_0[i][1] << " 0\n";
                    ofs1 << reprojected_1[i][0] << " "
                        << reprojected_1[i][1] << " 0\n";
                }
            }
            ofs0.close();
            ofs1.close();
            std::cout << "Reprojected 2D points saved to 'reprojected_0.xyz' and 'reprojected_1.xyz'\n";
        }
    }
